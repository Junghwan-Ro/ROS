# - Config file for the gz-physics6-tpelib component
#
# This should only be invoked by gz-physics6-config.cmake.
#
# To load this component into your project, use:
# find_package(gz-physics6 COMPONENTS tpelib)
#
# This creates the following targets:
#
#   Component library target                - gz-physics6::gz-physics6-tpelib
#   Alternative target name                 - gz-physics6::tpelib
...
# this is discouraged:
#
#   gz-physics6-tpelib_LIBRARY  - Component library (actually contains gz-physics6::tpelib)
#
# We will also set gz-physics6-tpelib_FOUND to indicate that the component was found.
#
################################################################################
...
if(gz-physics6-tpelib_CONFIG_INCLUDED)
  return()
endif()
set(gz-physics6-tpelib_CONFIG_INCLUDED TRUE)

if(NOT gz-physics6-tpelib_FIND_QUIETLY)
  message(STATUS "Looking for gz-physics6-tpelib -- found version 6.3.0")
endif()
...
# dependencies.
if(gz-physics6-tpelib_FIND_REQUIRED)
...
# dependencies.
if(gz-physics6-tpelib_FIND_QUIETLY)
...
if(NOT gz-physics6-tpelib_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-physics6-tpelib")
endif()
...
if(NOT TARGET gz-physics6::gz-physics6-tpelib)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-physics6-tpelib-targets.cmake")

  # Create a simplified imported target name for the tpelib library.
  # You can link to this target instead of the tpelib library.
  add_library(gz-physics6::tpelib INTERFACE IMPORTED)
  set_target_properties(gz-physics6::tpelib PROPERTIES
    INTERFACE_LINK_LIBRARIES "gz-physics6::gz-physics6-tpelib")
...
if(NOT gz_requested_components)
  ...
  set_target_properties(gz-physics6::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_requested_components};gz-physics6::gz-physics6-tpelib")
endif()

set(gz-physics6-tpelib_LIBRARY gz-physics6::gz-physics6-tpelib)

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-physics6-tpelib "gz-physics6-tpelib")
