# - Config file for the gz-sim8-diff-drive-system component
#
# This should only be invoked by gz-sim8-config.cmake.
#
# To load this component into your project, use:
# find_package(gz-sim8 COMPONENTS diff-drive-system)
#
# This creates the following targets:
#
#   Component library target                - gz-sim8::gz-sim8-diff-drive-system
#   Alternative target name                 - gz-sim8::diff-drive-system
...
# this is discouraged:
#
#   gz-sim8-diff-drive-system_LIBRARY  - Component library (actually contains gz-sim8::diff-drive-system)
#
# We will also set gz-sim8-diff-drive-system_FOUND to indicate that the component was found.
#
################################################################################
...
if(gz-sim8-diff-drive-system_CONFIG_INCLUDED)
  return()
endif()
set(gz-sim8-diff-drive-system_CONFIG_INCLUDED TRUE)

if(NOT gz-sim8-diff-drive-system_FIND_QUIETLY)
  message(STATUS "Looking for gz-sim8-diff-drive-system -- found version 8.0.0~pre1")
endif()
...
# dependencies.
if(gz-sim8-diff-drive-system_FIND_REQUIRED)
...
# dependencies.
if(gz-sim8-diff-drive-system_FIND_QUIETLY)
...
if(NOT gz-sim8-diff-drive-system_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-sim8-diff-drive-system")
endif()
...
if(NOT TARGET gz-sim8::gz-sim8-diff-drive-system)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-sim8-diff-drive-system-targets.cmake")
  ...
  add_library(gz-sim8::diff-drive-system INTERFACE IMPORTED)
  set_target_properties(gz-sim8::diff-drive-system PROPERTIES
    INTERFACE_LINK_LIBRARIES "gz-sim8::gz-sim8-diff-drive-system")
...
if(NOT gz_requested_components)
  ...
  set_target_properties(gz-sim8::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_requested_components};gz-sim8::gz-sim8-diff-drive-system")
endif()

set(gz-sim8-diff-drive-system_LIBRARY gz-sim8::gz-sim8-diff-drive-system)

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-sim8-diff-drive-system "gz-sim8-diff-drive-system")
