# - Config file for the gz-sensors8-dvl component
#
# This should only be invoked by gz-sensors8-config.cmake.
#
# To load this component into your project, use:
# find_package(gz-sensors8 COMPONENTS dvl)
#
# This creates the following targets:
#
#   Component library target                - gz-sensors8::gz-sensors8-dvl
#   Alternative target name                 - gz-sensors8::dvl
...
# this is discouraged:
#
#   gz-sensors8-dvl_LIBRARY  - Component library (actually contains gz-sensors8::dvl)
#
# We will also set gz-sensors8-dvl_FOUND to indicate that the component was found.
#
################################################################################
...
if(gz-sensors8-dvl_CONFIG_INCLUDED)
  return()
endif()
set(gz-sensors8-dvl_CONFIG_INCLUDED TRUE)

if(NOT gz-sensors8-dvl_FIND_QUIETLY)
  message(STATUS "Looking for gz-sensors8-dvl -- found version 8.0.0~pre1")
endif()
...
# dependencies.
if(gz-sensors8-dvl_FIND_REQUIRED)
...
# dependencies.
if(gz-sensors8-dvl_FIND_QUIETLY)
...
if(NOT gz-sensors8-dvl_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-sensors8-dvl")
endif()
if(NOT gz-sensors8_CONFIG_INCLUDED)
  find_package(gz-sensors8 8.0.0 EXACT ${gz_package_quiet} ${gz_package_required})
endif()
find_package(gz-sensors8 8.0.0 EXACT ${gz_package_quiet} ${gz_package_required} COMPONENTS rendering)
# --------------------------------

if(NOT TARGET gz-sensors8::gz-sensors8-dvl)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-sensors8-dvl-targets.cmake")

  # Create a simplified imported target name for the dvl library.
  # You can link to this target instead of the dvl library.
  add_library(gz-sensors8::dvl INTERFACE IMPORTED)
  set_target_properties(gz-sensors8::dvl PROPERTIES
    INTERFACE_LINK_LIBRARIES "gz-sensors8::gz-sensors8-dvl")
...
# Link the dvl library to the "requested" target
...
if(NOT gz_requested_components)
  ...
  set_target_properties(gz-sensors8::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_requested_components};gz-sensors8::gz-sensors8-dvl")
endif()

set(gz-sensors8-dvl_LIBRARY gz-sensors8::gz-sensors8-dvl)

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-sensors8-dvl "gz-sensors8-dvl")
