# - Config file for the gz-common5 package.
#
# For finding and loading gz-common5 from your project, type:
#
# find_package(gz-common5)
#
# This creates the following targets:
#
#   Core library target                     - gz-common5::gz-common5
#   Alternative target name                 - gz-common5::core
#   Core library + all requested components - gz-common5::requested
#
...
#
#   gz-common5_CORE_LIBRARY - Core library (actually contains gz-common5::core)
#   gz-common5_LIBRARIES    - All libraries to link against (actually contains gz-common5::requested).
#   gz-common5_INCLUDE_DIRS - Include directories for gz-common5 and its dependencies.
#
# We will also set gz-common5_FOUND to indicate that the package was found.
#
################################################################################
...
if(NOT gz-common5_FIND_QUIETLY)
  message(STATUS "Looking for gz-common5 -- found version 5.3.1")
endif()

if(gz-common5_CONFIG_INCLUDED)

  # Check that each of the components requested in this call to find_package(~)
  # have already been found.
  set(gz-common5_all_requested_components_found true)
  foreach(component ${gz-common5_FIND_COMPONENTS})
    if(NOT gz-common5-${component}_FOUND)
      set(gz-common5_all_requested_components_found false)
      break()
    endif()
  endforeach()

  # If this gz-common5-config.cmake file has been called before,
  # and all the requested components are already found, then exit early.
  if(gz-common5_all_requested_components_found)
    return()
  endif()
endif()
set(gz-common5_CONFIG_INCLUDED TRUE)
...
if(NOT gz-common5_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-common5")
endif()
find_package(gz-utils2 ${gz_package_quiet} ${gz_package_required})
find_package(DL ${gz_package_quiet} ${gz_package_required})
...
if(NOT TARGET gz-common5::gz-common5)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-common5-targets.cmake")

  # Create a simplified imported target name for the core library.
  # You can link to this target instead of the core library.
  add_library(gz-common5::core INTERFACE IMPORTED)
  set_target_properties(gz-common5::core PROPERTIES
  ...
  # effectively the same as gz-common5::core, but it does not have the
  # benefit of unambiguously being a target name.
  add_library(gz-common5 INTERFACE IMPORTED)
  set_target_properties(gz-common5 PROPERTIES
    INTERFACE_LINK_LIBRARIES gz-common5::gz-common5)
...
get_target_property(gz_all_components gz-common5::requested INTERFACE_LINK_LIBRARIES)
if(NOT gz_all_components)
  # If gz-common5::requested has not been given any libraries yet, then cmake will
  ...
  set_target_properties(gz-common5::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_all_components};gz-common5::gz-common5")
endif()

# Package variables. Note that gz-common5_LIBRARIES and gz-common5_CORE_LIBRARY
# contain imported targets, so gz-common5_INCLUDE_DIRS is never needed.
set(gz-common5_CORE_LIBRARY gz-common5::core)
set(gz-common5_LIBRARIES gz-common5::requested)
set_and_check(gz-common5_INCLUDE_DIRS "${PACKAGE_PREFIX_DIR}/include/gz/common5")

# Backwards compatibility variables
set(GZ-COMMON_LIBRARIES ${gz-common5_LIBRARIES})
set(GZ-COMMON_INCLUDE_DIRS ${gz-common5_INCLUDE_DIRS})

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-common5 "gz-common5")

# Find each of the components requested by find_package(~)
foreach(component ${gz-common5_FIND_COMPONENTS})

  if(NOT gz-common5_FIND_QUIETLY)
    message(STATUS "Searching for <gz-common5> component [${component}]")
  endif()

  if(gz-common5_FIND_REQUIRED_${component})
    ...
    # find_dependency(~)
    find_dependency(gz-common5-${component} 5.3.1 EXACT)
    ...
    # forwarded to it.
    find_package(gz-common5-${component} 5.3.1 EXACT ${gz_package_quiet})
...
set(GZ-COMMON_DOXYGEN_TAGFILE "${PACKAGE_PREFIX_DIR}/share/gz/gz-common5/gz-common5.tag.xml")

# Specify the API url. This is where the doxygen tag file will resolve URLS to.
set(GZ-COMMON_API_URL "https://gazebosim.org/api/common/5")
