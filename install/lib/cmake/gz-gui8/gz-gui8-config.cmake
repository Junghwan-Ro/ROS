# - Config file for the gz-gui8 package.
#
# For finding and loading gz-gui8 from your project, type:
#
# find_package(gz-gui8)
#
# This creates the following targets:
#
#   Core library target                     - gz-gui8::gz-gui8
#   Alternative target name                 - gz-gui8::core
#   Core library + all requested components - gz-gui8::requested
#
...
#
#   gz-gui8_CORE_LIBRARY - Core library (actually contains gz-gui8::core)
#   gz-gui8_LIBRARIES    - All libraries to link against (actually contains gz-gui8::requested).
#   gz-gui8_INCLUDE_DIRS - Include directories for gz-gui8 and its dependencies.
#
# We will also set gz-gui8_FOUND to indicate that the package was found.
#
################################################################################
...
if(NOT gz-gui8_FIND_QUIETLY)
  message(STATUS "Looking for gz-gui8 -- found version 8.0.0~pre1")
endif()

if(gz-gui8_CONFIG_INCLUDED)

  # Check that each of the components requested in this call to find_package(~)
  # have already been found.
  set(gz-gui8_all_requested_components_found true)
  foreach(component ${gz-gui8_FIND_COMPONENTS})
    if(NOT gz-gui8-${component}_FOUND)
      set(gz-gui8_all_requested_components_found false)
      break()
    endif()
  endforeach()

  # If this gz-gui8-config.cmake file has been called before,
  # and all the requested components are already found, then exit early.
  if(gz-gui8_all_requested_components_found)
    return()
  endif()
endif()
set(gz-gui8_CONFIG_INCLUDED TRUE)
...
if(NOT gz-gui8_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-gui8")
endif()
...
find_package(Qt5 ${gz_package_quiet} ${gz_package_required} COMPONENTS Core;Quick;QuickControls2;Widgets)
# --------------------------------
...
if(NOT TARGET gz-gui8::gz-gui8)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-gui8-targets.cmake")

  # Create a simplified imported target name for the core library.
  # You can link to this target instead of the core library.
  add_library(gz-gui8::core INTERFACE IMPORTED)
  set_target_properties(gz-gui8::core PROPERTIES
    INTERFACE_LINK_LIBRARIES gz-gui8::gz-gui8)
  ...
  # effectively the same as gz-gui8::core, but it does not have the
  # benefit of unambiguously being a target name.
  add_library(gz-gui8 INTERFACE IMPORTED)
  set_target_properties(gz-gui8 PROPERTIES
    INTERFACE_LINK_LIBRARIES gz-gui8::gz-gui8)
...
get_target_property(gz_all_components gz-gui8::requested INTERFACE_LINK_LIBRARIES)
if(NOT gz_all_components)
  # If gz-gui8::requested has not been given any libraries yet, then cmake will
  ...
  set_target_properties(gz-gui8::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_all_components};gz-gui8::gz-gui8")
endif()

# Package variables. Note that gz-gui8_LIBRARIES and gz-gui8_CORE_LIBRARY
# contain imported targets, so gz-gui8_INCLUDE_DIRS is never needed.
set(gz-gui8_CORE_LIBRARY gz-gui8::core)
set(gz-gui8_LIBRARIES gz-gui8::requested)
set_and_check(gz-gui8_INCLUDE_DIRS "${PACKAGE_PREFIX_DIR}/include/gz/gui8")

# Backwards compatibility variables
set(GZ-GUI_LIBRARIES ${gz-gui8_LIBRARIES})
set(GZ-GUI_INCLUDE_DIRS ${gz-gui8_INCLUDE_DIRS})

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-gui8 "gz-gui8")

# Find each of the components requested by find_package(~)
foreach(component ${gz-gui8_FIND_COMPONENTS})

  if(NOT gz-gui8_FIND_QUIETLY)
    message(STATUS "Searching for <gz-gui8> component [${component}]")
  endif()

  if(gz-gui8_FIND_REQUIRED_${component})
    ...
    # find_dependency(~)
    find_dependency(gz-gui8-${component} 8.0.0 EXACT)
    ...
    # forwarded to it.
    find_package(gz-gui8-${component} 8.0.0 EXACT ${gz_package_quiet})
...
set(GZ-GUI_DOXYGEN_TAGFILE "${PACKAGE_PREFIX_DIR}/share/gz/gz-gui8/gz-gui8.tag.xml")

# Specify the API url. This is where the doxygen tag file will resolve URLS to.
set(GZ-GUI_API_URL "https://gazebosim.org/api/gui/8")
